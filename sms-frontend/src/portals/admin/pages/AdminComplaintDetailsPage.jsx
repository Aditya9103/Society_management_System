...
import { 
    ArrowLeft, Download, Edit, AlertTriangle, MessageSquare, Clock, 
    CheckCircle, Archive, AlertCircle, ChevronDown, Check, Zap, 
    Droplets, Shield, HelpCircle, Settings, Volume2, Car, Wrench, Image as ImageIcon
} from 'lucide-react';
import { 
...
export default function AdminComplaintDetailsPage() {
    ...
    const [changeStatus, { isLoading: isUpdating }] = useChangeComplaintStatusAdminMutation();
    ...
    // Default the select inputs to current values when loaded
    if (!selectedStatus && complaint.status) setSelectedStatus(complaint.status);
    if (!selectedAssignee && complaint.assignedTo?._id) setSelectedAssignee(complaint.assignedTo._id);

    const handleUpdateStatus = async () => {
        try {
            const payload = { id: complaint._id, status: selectedStatus };
            if (statusNote) payload.notes = statusNote;
            if (selectedAssignee !== complaint.assignedTo?._id) {
                payload.assignedTo = selectedAssignee || null;
            }
    ...
    const handleQuickAction = async (actionStatus) => {
        try {
            await changeStatus({ id: complaint._id, status: actionStatus }).unwrap();
            ...
            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 text-[12px] font-bold text-gray-400 mb-2">
                        <Link to="/admin/complaints" className="hover:text-white transition-colors flex items-center gap-1">
                            <ArrowLeft className="w-3 h-3" /> Complaints
                        </Link>
                        <span>&gt;</span>
                ...
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 text-white text-[13px] font-bold border border-white/10 hover:bg-white/10 transition-colors shadow-sm">
                        <Download className="w-4 h-4" /> Export Report
                    </button>
                    <button className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#6338f0] hover:bg-[#5b32e6] text-white text-[13px] font-bold transition-all shadow-[0_0_15px_rgba(99,56,240,0.3)]">
                        <Edit className="w-4 h-4" /> Edit Complaint
                    </button>
                    {complaint.status === 'OPEN' && (
                        <button 
                            onClick={() => handleQuickAction('ESCALATED')}
                            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500 hover:bg-red-600 text-white text-[13px] font-bold transition-all shadow-[0_0_15px_rgba(239,68,68,0.3)]"
                        >
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-[#151722] border border-white/5 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row gap-6 md:gap-10">
                        ...
                        <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div>
                                <span className="text-[11px] font-bold text-gray-500">Reported By</span>
                                <div className="flex items-center gap-2 mt-2">
                                    <div className="w-6 h-6 rounded-full bg-indigo-500/20 flex items-center justify-center text-[10px] text-indigo-400 font-bold overflow-hidden shrink-0">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[13px] font-bold text-white">{raisedBy.firstName} {raisedBy.lastName}</span>
                                        <span className="text-[11px] font-bold text-gray-400">Unit {unit} &bull; {raisedBy.phone}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="bg-[#151722] border border-white/5 rounded-2xl p-6 shadow-sm">
                        <h3 className="text-[16px] font-bold text-white mb-4">Update Status & Add Note</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                <label className="block text-[11px] font-bold text-gray-500 mb-1.5">Update Status</label>
                                <div className="relative">
                                    <select 
                                        value={selectedStatus}
                                        onChange={(e) => setSelectedStatus(e.target.value)}
                                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] text-white font-bold focus:outline-none focus:border-[#6338f0]/50 appearance-none shadow-sm"
                                    >
                                ...
                                <label className="block text-[11px] font-bold text-gray-500 mb-1.5 mt-4">Assign Staff</label>
                                <div className="relative">
                                    <select 
                                    ...
                                    >
                                        ...
                                        {staff.filter(s => ['HELP_DESK', 'FACILITY_MANAGER', 'MAINTENANCE'].includes(s.role)).map(s => (
                            ...
                            <div className="md:col-span-2 flex flex-col h-full">
                                <label className="block text-[11px] font-bold text-gray-500 mb-1.5">Add Note</label>
                                <textarea 
                                    value={statusNote}
                                    onChange={(e) => setStatusNote(e.target.value)}
                                    placeholder="Add a note about the progress..."
                                    className="flex-1 min-h-[100px] bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-[13px] text-white font-bold placeholder-gray-500 focus:outline-none focus:border-[#6338f0]/50 shadow-sm resize-none"
                                />
                                <div className="flex items-center justify-between mt-3">
                                    <span className="text-[11px] font-bold text-gray-500">This note will be visible to all assigned staff.</span>
                                    <button 
                                        onClick={handleUpdateStatus}
                                        disabled={isUpdating}
                                        className="px-6 py-2.5 bg-[#6338f0] hover:bg-[#5b32e6] text-white text-[13px] font-bold rounded-xl transition-colors shadow-[0_0_15px_rgba(99,56,240,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-[#151722] border border-white/5 rounded-2xl p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-5">
                            ...
                            <button className="text-[11px] font-bold text-[#6338f0] bg-[#6338f0]/10 px-3 py-1 rounded-full border border-[#6338f0]/20">View All</button>
                        </div>
                            ...
                            <div className="relative pl-6">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center justify-between mt-1">
                                        <span className="text-[12px] font-bold text-gray-400">{raisedBy.firstName} {raisedBy.lastName}</span>
                                    ...
                                    <p className="text-[12px] font-bold text-gray-400 mt-2 line-clamp-2">
                                        {complaint.description}
                                    </p>
                                </div>
                            </div>
                                ...
                                <div key={event._id || idx} className="relative pl-6">
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-[13px] font-bold text-white">
                                            {event.statusChangedTo ? `Status changed to ${event.statusChangedTo}` : 'Note Added'}
                                        </span>
                                        <div className="flex items-center justify-between mt-1">
                                            <span className="text-[12px] font-bold text-gray-400">
                                                {event.authorId?.firstName} {event.authorId?.lastName}
                                            </span>
                                        ...
                                        {event.commentText && (
                                            <p className="text-[12px] font-bold text-gray-300 mt-2 bg-white/5 p-2 rounded-lg border border-white/10">
                                                {event.commentText}
                                            </p>
                                        )}
                                    </div>
                    ...
                    {/* Quick Actions */}
                    <div className="bg-[#151722] border border-white/5 rounded-2xl p-6 shadow-sm">
                        <h3 className="text-[16px] font-bold text-white mb-4">Quick Actions</h3>
                        <div className="space-y-3">
                            <button 
                                onClick={() => handleQuickAction('IN_PROGRESS')}
                                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 text-blue-400 text-[13px] font-bold transition-all group"
                            >
                                <Clock className="w-4 h-4 group-hover:scale-110 transition-transform" /> Mark as In Progress
                            </button>
                            <button 
                                onClick={() => handleQuickAction('RESOLVED')}
                                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-[13px] font-bold transition-all group"
                            >
                                <CheckCircle className="w-4 h-4 group-hover:scale-110 transition-transform" /> Mark as Resolved
                            </button>
                            <button 
                                onClick={() => handleQuickAction('CLOSED')}
                                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-gray-300 text-[13px] font-bold transition-all group"
                            >
                                <Archive className="w-4 h-4 group-hover:scale-110 transition-transform" /> Close Complaint
                            </button>
                        </div>
                    </div>
                </div>
