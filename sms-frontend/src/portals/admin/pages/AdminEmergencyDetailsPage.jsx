...
import { ShieldAlert, ArrowLeft, Mic, UserPlus, CheckCircle, MapPin, ExternalLink, Activity, Clock, Users, Shield, Calendar, Phone, PenSquare, Share2, Info } from 'lucide-react';
import { useGetEmergencyByIdQuery, useUpdateEmergencyStatusMutation, useAssignEmergencyStaffMutation, useBroadcastUpdateMutation } from '../../../store/api/emergencyApi';
...
export default function AdminEmergencyDetailsPage() {
    const { id } = useParams();
    const navigate = useNavigate();
    
    const { data: res, isLoading } = useGetEmergencyByIdQuery(id);
    const { data: staffRes } = useListStaffQuery();
    
    const [updateStatus, { isLoading: isUpdating }] = useUpdateEmergencyStatusMutation();
    const [assignStaff, { isLoading: isAssigning }] = useAssignEmergencyStaffMutation();
    ...
    if (!emergency) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[600px] text-slate-500">
                ...
                <button onClick={() => navigate('/admin/emergencies')} className="text-[#6338f0] hover:underline">Back to Emergencies</button>
            </div>
        );
    }
    ...
    const handleResolve = async () => {
        try {
            await updateStatus({ id, status: 'RESOLVED', resolutionNotes: 'Resolved by admin from dashboard' }).unwrap();
    ...
    const handleAssignStaff = async () => {
        ...
        try {
            await assignStaff({ id, staffIds: selectedStaffIds }).unwrap();
    ...
    return (
        <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
            {/* Header section */}
            <div>
                <button onClick={() => navigate('/admin/emergencies')} className="flex items-center gap-2 text-slate-400 hover:text-white mb-4 transition-colors">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => setShowBroadcastModal(true)} className="flex items-center justify-center gap-2 bg-[#131525] border border-slate-700 text-slate-300 px-4 py-2 rounded-lg text-sm hover:border-slate-600 transition-colors">
                            <Mic className="w-4 h-4" /> Broadcast Alert
                        </button>
                        <button onClick={() => setShowAssignModal(true)} className="flex items-center justify-center gap-2 bg-[#131525] border border-slate-700 text-slate-300 px-4 py-2 rounded-lg text-sm hover:border-slate-600 transition-colors">
                            <UserPlus className="w-4 h-4" /> Assign Staff
                        </button>
                        {emergency.status !== 'RESOLVED' && emergency.status !== 'FALSE_ALARM' && (
                            <button onClick={handleResolve} disabled={isUpdating} className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div id="team-section" className="bg-[#1a1c29] border border-slate-800/50 rounded-2xl flex flex-col">
                            ...
                            <button onClick={() => setShowAssignModal(true)} className="p-4 text-[#6338f0] text-sm font-medium hover:text-indigo-400 transition text-center border-t border-slate-800/50 bg-[#131525]/30 rounded-b-2xl">
                                View All Assigned Staff &rarr;
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {emergency.status === 'ACTIVE' && priorityInfo.label === 'High Priority' && (
                <div className="bg-red-900/20 border border-red-500/30 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-center gap-4 mt-6">
                    ...
                    <button className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold transition whitespace-nowrap shadow-lg shadow-red-900/50 flex items-center gap-2">
                        <Activity className="w-5 h-5" /> Take Immediate Action
                    </button>
                </div>
            )}
            ...
            {showAssignModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0b0e14]/80 backdrop-blur-sm p-4">
                    <div className="bg-[#1a1c29] border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-slate-800">
                            <h2 className="text-xl font-bold text-white">Assign Staff to Emergency</h2>
                            <p className="text-sm text-slate-400 mt-1">Select staff members to respond to this incident.</p>
                        </div>
                        <div className="p-6 max-h-[400px] overflow-y-auto space-y-2">
                            {allStaff.filter(s => ['SECURITY_GUARD', 'FACILITY_MANAGER'].includes(s.role)).map(staff => (
                                <label key={staff._id} className="flex items-center gap-3 p-3 rounded-xl border border-slate-800 hover:bg-[#131525] cursor-pointer transition">
                                    <input 
                                        type="checkbox" 
                                        ...
                                        onChange={(e) => {
                                            if (e.target.checked) setSelectedStaffIds([...selectedStaffIds, staff._id]);
                                            else setSelectedStaffIds(selectedStaffIds.filter(id => id !== staff._id));
                                        }}
                                        disabled={emergency.responders?.some(r => r.userId?._id === staff._id)}
                                    />
                                    <div className="flex-1">
                                        <div className="text-sm font-bold text-white">{staff.firstName} {staff.lastName}</div>
                                        <div className="text-xs text-slate-500">{staff.role.replace('_', ' ')}</div>
                                    </div>
                                    {emergency.responders?.some(r => r.userId?._id === staff._id) && (
                                        <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-1 rounded">Assigned</span>
                                    )}
                                </label>
                            ))}
                        </div>
                        <div className="px-6 py-4 bg-[#131525] border-t border-slate-800 flex justify-end gap-3">
                            <button onClick={() => setShowAssignModal(false)} className="px-4 py-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg font-medium transition">
                                Cancel
                            </button>
                            <button 
                                onClick={handleAssignStaff} 
                                disabled={selectedStaffIds.length === 0 || isAssigning}
                                className="px-4 py-2 bg-[#6338f0] hover:bg-[#5225e2] text-white rounded-lg font-medium transition disabled:opacity-50"
                            >
                    ...
                    <div className="bg-[#1a1c29] border border-slate-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="px-6 py-4 bg-[#131525] border-t border-slate-800 flex justify-end gap-3">
                            <button
                                ...
                                className="px-4 py-2 bg-transparent border-slate-700 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg font-medium transition"
                            >
                                Cancel
                            </button>
                            <button
