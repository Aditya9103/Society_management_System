    ...
    return (
        <div className="space-y-6 pb-10 font-sans">
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#151722] text-white font-bold text-sm font-semibold border border-white/5 hover:bg-white/5 hover:text-white transition-colors">
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-center bg-[#11131c] p-3 rounded-2xl border border-white/5">
                <div className="relative flex-1 w-full">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white font-bold" />
                    <input
                        type="text"
                ...
                <div className="flex gap-3 w-full lg:w-auto">
                    ...
                    <button onClick={refetch} className="flex items-center justify-center w-11 h-11 rounded-xl bg-[#151722] text-white font-bold border border-white/5 hover:bg-white/5 transition-colors">
            ...
            <div className="md:hidden space-y-4">
                {isLoading ? (
                    <div className="py-8 text-center text-white font-bold">Loading...</div>
                ) : residents.length === 0 ? (
                    <div className="py-8 text-center text-white font-bold">No residents found.</div>
                ) : residents.map(r => (
                    <div key={r._id} onClick={() => setSelectedResidentId(r.userId?._id)} className="bg-[#11131c] rounded-2xl border border-white/5 p-4 flex flex-col gap-3 active:scale-[0.99] transition-transform">
                        <div className="flex justify-between items-start">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="text-white text-base font-bold">{r.userId?.firstName} {r.userId?.lastName}</div>
                                    <div className="text-white font-bold text-xs font-bold mt-0.5">{r.userId?.phone || 'No phone'}</div>
                                </div>
                            </div>
                            <div className="text-right">
                                {r.approvalStatus === 'APPROVED' ? (
                                    <span className="px-2 py-0.5 rounded-md bg-[#052e16] border border-[#166534] text-[#4ade80] text-[12px] font-bold tracking-wide">APPROVED</span>
                                ) : r.approvalStatus === 'PENDING' ? (
                                    <span className="px-2 py-0.5 rounded-md bg-[#422006] border border-[#713f12] text-[#fbbf24] text-[12px] font-bold tracking-wide">PENDING</span>
                                ) : (
                                    <span className="px-2 py-0.5 rounded-md bg-[#4c0519] border border-[#881337] text-[#f87171] text-[12px] font-bold tracking-wide">REJECTED</span>
                                )}
                                <div className="text-white font-bold text-[12px] mt-1">{new Date(r.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</div>
                            </div>
                        </div>
                        <div className="flex items-center justify-between bg-[#151722] rounded-xl p-3 border border-white/5">
                            <div>
                                <div className="text-white font-bold text-[12px] font-semibold uppercase mb-0.5">Unit</div>
                                <div className="text-white text-sm font-bold">{r.unitId?.unitNumber || 'N/A'} (Tower {r.unitId?.towerId})</div>
                            </div>
                            <div className="text-right">
                                <div className="text-white font-bold text-[12px] font-semibold uppercase mb-0.5">Role</div>
                                <div className="text-[#b388ff] text-sm font-bold">{r.ownershipType === 'OWNER' ? 'Owner' : 'Tenant'}</div>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <div className="hidden md:block bg-[#11131c] rounded-2xl border border-white/5 overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr className="border-b border-white/5">
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Resident</th>
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Unit Details</th>
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Contact</th>
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Status</th>
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Joined On</th>
                            <th className="py-4 px-6 text-xs font-bold text-white font-bold uppercase tracking-wider whitespace-nowrap">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                        {isLoading ? (
                            <tr><td colSpan="6" className="py-8 text-center text-white font-bold">Loading...</td></tr>
                        ) : residents.length === 0 ? (
                            <tr><td colSpan="6" className="py-8 text-center text-white font-bold">No residents found.</td></tr>
                        ) : residents.map(r => (
                            <tr key={r._id} onClick={() => setSelectedResidentId(r.userId?._id)} className="hover:bg-[#151722]/50 transition-colors cursor-pointer group">
                                <td className="py-4 px-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-white text-sm font-bold">{r.userId?.firstName} {r.userId?.lastName}</div>
                                            <div className="text-white font-bold text-xs font-bold mt-0.5">ID: {r.residentCode || r._id.slice(-6).toUpperCase()}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="py-4 px-6">
                                    <div className="text-white text-sm font-bold">{r.unitId?.unitNumber || 'N/A'}</div>
                                    <div className="text-white font-bold text-xs font-bold mt-0.5">Tower {r.unitId?.towerId} • {r.unitId?.bhkType || 'N/A'}</div>
                                    <div className="mt-1.5 inline-block px-2 py-0.5 rounded-[4px] bg-[#6338f0]/10 text-[#b388ff] text-xs font-bold">
                                        {r.ownershipType === 'OWNER' ? 'Owner' : 'Tenant'}
                                    </div>
                                </td>
                                <td className="py-4 px-6">
                                    <div className="flex items-center gap-2 text-white font-bold text-sm font-bold">
                                        <Mail className="w-4 h-4 text-[#b388ff]" />
                                        {r.userId?.email}
                                    </div>
                                    <div className="flex items-center gap-2 text-white font-bold text-sm font-bold mt-1">
                                        <Phone className="w-4 h-4 text-[#b388ff]" />
                                        {r.userId?.phone || 'N/A'}
                                    </div>
                                </td>
                                <td className="py-4 px-6">
                                    {r.approvalStatus === 'APPROVED' ? (
                                        <span className="px-2.5 py-1 rounded-md bg-[#052e16] border border-[#166534] text-[#4ade80] text-xs font-bold tracking-wide">APPROVED</span>
                                    ) : r.approvalStatus === 'PENDING' ? (
                                        <span className="px-2.5 py-1 rounded-md bg-[#422006] border border-[#713f12] text-[#fbbf24] text-xs font-bold tracking-wide">PENDING</span>
                                    ) : (
                                        <span className="px-2.5 py-1 rounded-md bg-[#4c0519] border border-[#881337] text-[#f87171] text-xs font-bold tracking-wide">REJECTED</span>
                                    )}
                                </td>
                                <td className="py-4 px-6 text-white font-bold text-sm font-bold whitespace-nowrap">
                ...
                <div className="px-6 py-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#11131c] rounded-2xl md:rounded-t-none md:rounded-b-2xl border border-white/5 md:border-t-0 mt-4 md:mt-0">
                    <div className="text-white font-bold text-[12px] font-bold">
                    ...
                    {pagination?.totalPages > 1 && (
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                className="w-8 h-8 flex items-center justify-center rounded-lg text-white font-bold hover:bg-white/5 hover:text-white disabled:opacity-50"
                            >
                                &laquo;
                            </button>
                            <button
                                ...
                                if (p === 1 || p === pagination.totalPages || (p >= page - 1 && p <= page + 1)) {
                                    return (
                                        <button
                                            key={p}
                                            onClick={() => setPage(p)}
                                            className={`w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold ${page === p ? 'bg-[#6338f0] text-white' : 'text-white font-bold hover:bg-white/5 hover:text-white'}`}
                                        >
                                            {p}
                                        </button>
                                    );
                    ...
                    <div className="flex items-center gap-2 text-[12px] text-white font-bold font-bold">
                        Rows per page:
                        <select
                            value={limit}
