...
export function DarkModal({ isOpen, onClose, title, children }) {
    ...
    return createPortal(
        <div className="fixed inset-0 flex items-center justify-center sm:p-4 z-[9999]">
            <div className="absolute inset-0 bg-black/80 backdrop-blur-md" onClick={onClose} />
            <div className="relative z-10 w-full max-w-md h-[100dvh] sm:h-auto rounded-none sm:rounded-3xl bg-[#0B0D17] border-0 sm:border border-indigo-500/20 shadow-[0_0_40px_rgba(99,102,241,0.15)] overflow-hidden flex flex-col sm:max-h-[90vh]">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 text-white pb-32 sm:pb-5">
                    {children}
                </div>
            </div>
        </div>,
...
export function DarkInput({ label, error, ...props }) {
    return (
        <div className="w-full">
            ...
            <input 
                className={`w-full rounded-xl border border-white/10 bg-[#131525] px-4 py-3 text-sm text-white focus:outline-none focus:border-[#6338f0] focus:ring-1 focus:ring-[#6338f0] transition-colors shadow-inner ${error ? 'border-red-500' : ''}`}
...
export function DarkSelect({ label, error, children, ...props }) {
    return (
        <div className="w-full relative">
            {label && <label className="block text-xs font-bold text-white font-bold mb-2 tracking-wide">{label}</label>}
            <select 
                className={`w-full rounded-xl border border-white/10 bg-[#131525] px-4 py-3 text-sm text-white focus:outline-none focus:border-[#6338f0] focus:ring-1 focus:ring-[#6338f0] transition-colors appearance-none shadow-inner ${error ? 'border-red-500' : ''}`}
                style={{ colorScheme: 'dark' }}
                {...props}
            >
                {children}
...
export function DarkButton({ children, isLoading, variant = 'primary', className = '', ...props }) {
    ...
    const variants = {
        primary: "bg-[#6338f0] text-white hover:bg-[#5229db] shadow-[0_0_15px_rgba(99,56,240,0.3)] hover:shadow-[0_0_25px_rgba(99,56,240,0.5)]",
