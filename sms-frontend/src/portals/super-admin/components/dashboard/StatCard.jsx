...
export default function StatCard({ label, value, icon: Icon, color, subLabel, subValue }) {
    // Extract color name from string like "bg-blue-500" if provided this way, else use default
    const colorKey = typeof color === 'string' && color.startsWith('bg-') ? color.split('-')[1] : color;
    const colors = GRADIENTS[colorKey] || GRADIENTS.slate;
    const iconBg = ICON_BGS[colorKey] || ICON_BGS.slate;
    ...
    return (
        <div className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${colors} p-5 border border-white/5 shadow-lg`}>
            ...
            <div className="relative z-10">
                ...
                <div className="text-white text-3xl font-black mb-1 tracking-tight">{value ?? <span className="text-gray-500">—</span>}</div>
                {subLabel && (
